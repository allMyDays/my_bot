...
class RoleCreateCommandTest {
    ...
    private RoleCreateCommand roleCreateCommand;

    @BeforeEach
    void setUp() {
        MockitoAnnotations.openMocks(this);

        roleCreateCommand = new RoleCreateCommand(vkChatClient, roleService, messageMapper);
    ...
    @Test
    void shouldCreateRoleSuccess() throws ClientException, ApiException {
        when(commandMessage.getFirstRowArguments())
                .thenReturn(new String[]{"5", "Новая", "роль"});
        ...
        try (MockedStatic<TextUtils> textUtilsMock = mockStatic(TextUtils.class)) {
            ...
            createdRole.setRolePriority(ROLE_PRIORITY);
            createdRole.setRoleName(ROLE_NAME);

            when(roleService.createRole(eq(CHAT_ID), eq(FROM_ID), eq(ROLE_PRIORITY), eq(ROLE_NAME)))
            ...
            CommandExecutionStatus status = roleCreateCommand.execute(commandMessage);

            assertEquals(CommandExecutionStatus.SUCCESS, status);
            verify(roleService).createRole(CHAT_ID, FROM_ID, ROLE_PRIORITY, ROLE_NAME);
            ...
            assertEquals("✅Вы успешно создали новую роль «Новая роль» с приоритетом 5.", actual);
        }
    }
    ...
    void shouldReturnArgumentValidationErrorWhenNotEnoughArguments() throws ClientException, ApiException {
        ...
        verify(roleService, never()).createRole(anyLong(), anyLong(), anyInt(), anyString());
    }

    @Test
    void shouldReturnArgumentValidationErrorWhenInvalidPriority() throws ClientException, ApiException {
        when(commandMessage.getFirstRowArguments()).thenReturn(new String[]{"abc", "Новая", "роль"});
    ...
    void shouldReturnBusinessLogicErrorOnRoleException() throws ClientException, ApiException {
        ...
        try (MockedStatic<TextUtils> textUtilsMock = mockStatic(TextUtils.class)) {
            ...
            RoleException roleException = new RoleException("Ошибка создания роли") {};
            when(roleService.createRole(eq(CHAT_ID), eq(FROM_ID), eq(ROLE_PRIORITY), eq(ROLE_NAME)))
    ...
    void shouldPropagateClientExceptionFromSendText() throws ClientException, ApiException {
        ...
        try (MockedStatic<TextUtils> textUtilsMock = mockStatic(TextUtils.class)) {
            ...
            assertThrows(ClientException.class, () -> roleCreateCommand.execute(commandMessage));
            verify(vkChatClient).sendText(sendMessageDto);
        }
    }
    ...
    void shouldPropagateApiExceptionFromSendText() throws ClientException, ApiException {
        ...
        try (MockedStatic<TextUtils> textUtilsMock = mockStatic(TextUtils.class)) {
            ...
            assertThrows(ApiException.class, () -> roleCreateCommand.execute(commandMessage));
            verify(vkChatClient).sendText(sendMessageDto);
        }
    }
    ...
    void shouldPropagateRuntimeExceptionFromRoleService() throws ClientException, ApiException {
        ...
        try (MockedStatic<TextUtils> textUtilsMock = mockStatic(TextUtils.class)) {
            ...
            assertThrows(RuntimeException.class, () -> roleCreateCommand.execute(commandMessage));
