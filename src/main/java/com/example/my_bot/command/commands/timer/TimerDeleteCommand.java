...
public class TimerDeleteCommand implements ChatCommand {
    ...
    public void execute(CommandMessageDto messageDto) throws ClientException, ApiException {
        ...
        int timerId = Integer.parseInt(args[0]);

        if(timerId<1||timerId>(timers = timerService.getChatTimersSortedByIdAsc(chatId)).size()){
            vkChatClient.sendText("Не найдено таймера с таким ID.",peerId,true);
            return;
        }
        try{
            timerService.deleteTimerById(timers.get(timerId-1).getId());
        }catch (TimerException e){
          vkChatClient.sendText(e.getMessage(), peerId,true);
          return;
        } vkChatClient.sendText("✅Таймер с ID %d был успешно удалён.".formatted(timerId),peerId, true);
    }
}
